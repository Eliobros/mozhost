...
const CMS_API_URL = "http://localhost:3001" // URL do seu CMS na VPS
...
export async function POST(req: Request) {
  try {
    ...
    try {
      const cmsResponse = await fetch(`${CMS_API_URL}/create-container`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          // Adicione um token de autenticação para o CMS aqui, se necessário (recomendado!)
          // 'Authorization': `Bearer ${process.env.MOZHOST_BEARER_TOKEN}`
        },
        body: JSON.stringify({
          serverId: newServer._id.toString(), // Passa o ID do servidor para o CMS
          image,
          name: `mozhost-${newServer._id.toString()}`, // Nome único para o contêiner Docker
          ports,
          allocatedCpu,
          allocatedMemory,
          // Outros parâmetros que o CMS possa precisar, como fileSystemPath
        }),
      })
