...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!oldPath || !newPath) {
      return NextResponse.json({ error: "oldPath e newPath são obrigatórios." }, { status: 400 })
    }
    ...
    // 3. Chamar o CMS para renomear
    try {
      const cmsResponse = await fetch(`${CMS_API_URL}/files/${server._id}/rename`, {
        method: "POST",
        headers: {
      ...
      if (!cmsResponse.ok) {
        ...
        console.error("Erro ao chamar CMS para renomear:", errorData)
        return NextResponse.json(
          { error: `Erro ao renomear: ${errorData.message || "Erro desconhecido do CMS"}` },
          { status: 500 },
        )
      }
      ...
      console.error("Erro de conexão com o CMS ao renomear:", cmsError)
      return NextResponse.json(
    ...
    console.error("Erro na API de renomear arquivo/diretório:", error)
