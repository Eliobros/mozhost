...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { filePath, content } = await req.json() // Caminho do arquivo e novo conteúdo

    if (!filePath || content === undefined) {
      return NextResponse.json({ error: "filePath e content são obrigatórios." }, { status: 400 })
    }
    ...
    // 3. Chamar o CMS para salvar o arquivo
    try {
      const cmsResponse = await fetch(`${CMS_API_URL}/save-file/${server._id}`, {
        method: "POST",
        headers: {
      ...
      if (!cmsResponse.ok) {
        ...
        console.error("Erro ao chamar CMS para salvar arquivo:", errorData)
        return NextResponse.json(
          { error: `Erro ao salvar arquivo: ${errorData.message || "Erro desconhecido do CMS"}` },
          { status: 500 },
        )
      }
      ...
      console.error("Erro de conexão com o CMS ao salvar arquivo:", cmsError)
      return NextResponse.json(
    ...
    console.error("Erro na API de salvar arquivo:", error)
