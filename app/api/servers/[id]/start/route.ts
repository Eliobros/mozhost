...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (server.status === "running") {
      return NextResponse.json({ message: "Servidor já está rodando." }, { status: 200 })
    }

    // 3. Chamar o CMS para iniciar o contêiner
    try {
      const cmsResponse = await fetch(`${CMS_API_URL}/servers/${server.dockerContainerId}/start`, {
        method: "POST",
        headers: {
      ...
      if (!cmsResponse.ok) {
        ...
        console.error("Erro ao chamar CMS para iniciar:", errorData)
        server.status = "error" // Marcar como erro se o CMS falhar
        await server.save()
        return NextResponse.json(
          { error: `Erro ao iniciar servidor: ${errorData.message || "Erro desconhecido do CMS"}` },
          { status: 500 },
        )
      }
      ...
      return NextResponse.json({ message: "Servidor iniciado com sucesso." }, { status: 200 })
    } catch (cmsError: any) {
      console.error("Erro de conexão com o CMS ao iniciar servidor:", cmsError)
      server.status = "error"
      await server.save()
      return NextResponse.json(
    ...
    console.error("Erro na API de iniciar servidor:", error)
