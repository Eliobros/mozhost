...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    // 3. Chamar o CMS para reiniciar o contêiner
    try {
      const cmsResponse = await fetch(`${CMS_API_URL}/servers/${server.dockerContainerId}/restart`, {
        method: "POST",
        headers: {
      ...
      if (!cmsResponse.ok) {
        ...
        console.error("Erro ao chamar CMS para reiniciar:", errorData)
        server.status = "error" // Marcar como erro se o CMS falhar
        await server.save()
        return NextResponse.json(
          { error: `Erro ao reiniciar servidor: ${errorData.message || "Erro desconhecido do CMS"}` },
          { status: 500 },
        )
      }

      // 4. Atualizar status no MongoDB
      server.status = "running" // Assumimos que reiniciar o deixa rodando
      await server.save()

      return NextResponse.json({ message: "Servidor reiniciado com sucesso." }, { status: 200 })
    } catch (cmsError: any) {
      console.error("Erro de conexão com o CMS ao reiniciar servidor:", cmsError)
      server.status = "error"
      await server.save()
      return NextResponse.json(
    ...
    console.error("Erro na API de reiniciar servidor:", error)
